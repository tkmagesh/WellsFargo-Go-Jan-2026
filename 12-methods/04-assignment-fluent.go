...
func (li *LineItem) Units(count int) *LineItem {
	li.Count = count
	return li
}
...
func (sc *ShoppingCart) With(li *LineItem) *ShoppingCart {
...
func (sc *ShoppingCart) ApplyDiscount(discountPercentage float64) *ShoppingCart {
...
func FindProduct(id int) *LineItem {
	return NewLineItem(productMaster[id], 0)
}

func main() {

	sc := NewShoppingCart().
		With(FindProduct(101).Units(10)).
		With(FindProduct(102).Units(5)).
		With(FindProduct(103).Units(6)).
		ApplyDiscount(10)
